...
// 模型文件路径
static const string model_file = "../model/yolov5s.onnx";
//static const string model_file = R"(G:\ML\Yolo\yolov5\models\onnx_model\yolov5s.onnx)";
// 测试视频路径
static const string video_file = "../data/pedtracking.mp4";
//static const string video_file = R"(D:\Programs\MATLAB\R2018a\toolbox\driving\drivingdata\pedtracking.mp4)";

//static const string xml_file = R"(G:\ML\Yolo\yolov5\OpenvinoModel_WithoutPreprocess\yolov5s.xml)";
//static const string bin_file = R"(G:\ML\Yolo\yolov5\OpenvinoModel_WithoutPreprocess\yolov5s.bin)";
...
int main(int argc, char **argv)
{
    try {
        ...
        cap.open(video_file);
        if (!cap.isOpened()) {
            cout << "Exit! video fails to open!" << endl;
            return -1;
        }

        // 获取输入节点tensor信息
        Tensor input_image_tensor = infer_request.get_tensor("images");
        Shape input_image_tensor_shape = input_image_tensor.get_shape();
        const int input_h = input_image_tensor_shape[2]; //获得"images"节点的Height
        const int input_w = input_image_tensor_shape[3]; //获得"images"节点的Width
        cout << "input_h:" << input_h << "; input_w:" << input_w << endl;
        cout << "input_image_tensor's element type:" << input_image_tensor.get_element_type() << endl;
        cout << "input_image_tensor's shape:" << input_image_tensor_shape << endl;
        // 获取输出节点tensor信息
        Tensor output_tensor = infer_request.get_tensor("output");
        Shape output_tensor_shape = output_tensor.get_shape();
        const int out_rows = output_tensor_shape[1]; //获得"output"节点的out_rows
        const int out_cols = output_tensor_shape[2]; //获得"output"节点的out_cols
        ...
        while (true) {
            ...
            // 缩放, 使图片宽高与模型输入一致(512*512), 精度变为32f
            cv::Mat blob_image;
